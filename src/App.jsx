...
function loadItems() {
  try {
    ...
    if (saved) {
      ...
      // Merge: use saved for structure overrides (name, type, active, days)
      // but always include any NEW items added to itemsData
      const savedMap = {};
      parsed.forEach(i => { savedMap[`${i.category}||${i.name}`] = i; });
      // Start with itemsData, apply saved overrides
      const merged = itemsData.map(item => {
        const key = `${item.category}||${item.name}`;
        return savedMap[key] ? { ...item, ...savedMap[key] } : item;
      });
      // Add any custom items that exist in saved but not in itemsData
      parsed.forEach(item => {
        const key = `${item.category}||${item.name}`;
        const inBase = itemsData.find(i => `${i.category}||${i.name}` === key);
...
export default function App() {
  ...
  // Wrap setItems to also persist to localStorage
  ...
  async function loadAll() {
    try {
      ...
      // Store docId so we can delete/update specific records
  ...
  // Update a specific record's items (for fixing wrong entries)
  ...
  async function saveInventory(selectedDay) {
    try {
      ...
      await addDoc(collection(db, "inventoryHistory"), {
        ...
        createdAt:   new Date(),
        savedBy:     userName || "—",
        selectedDay: selectedDay, // record which day was counted
        items:       itemsToSave,
      });
      setCounts({});
      await reloadHistory();
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, padding: "16px 14px", paddingBottom: `calc(var(--nav-h) + 16px)`, overflowY: "auto" }}>
        {page === "Count"       && <CountPage       t={t} items={items} counts={counts} onCountChange={handleCountChange} onSave={saveInventory} historyData={historyData} onDeleteRecord={deleteRecord} onUpdateRecord={updateRecord} />}
